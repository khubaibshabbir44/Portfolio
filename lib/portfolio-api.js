...
const DEFAULTS_PATH = path.join(ROOT, 'js', 'training-defaults.js');
...
function loadDefaultTrainingData() {
  const source = fs.readFileSync(DEFAULTS_PATH, 'utf8');
  const context = {};
  vm.createContext(context);
  vm.runInContext(`${source}; this.defaults = getDefaultTrainingData();`, context);
  return context.defaults.map(normalizeTrainingRecord);
}
...
async function getRecords() {
  try {
    ...
    upsertRecords(defaults).catch((err) => console.error('Seeding failed:', err.message));
    return defaults;
  } catch (err) {
    console.error('Database unavailable, falling back to defaults:', err.message);
    return loadDefaultTrainingData();
  }
}
...
async function handleRecords(req, res) {
  if (req.method === 'GET') {
    ...
    return sendJson(res, 200, { records }, { 'Cache-Control': 'public, s-maxage=300, stale-while-revalidate=86400' });
  }
  if (req.method === 'POST') {
